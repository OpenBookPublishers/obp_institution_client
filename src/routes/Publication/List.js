...
import { Input, Card, Button, Table, filterDropdown, Icon, Divider } from 'antd';
...
export default class SearchList extends Component {
  ...
  hadleTitleSearch = (selectedKeys, confirm) => () => {
    confirm();
    this.setState({ searchTitle: selectedKeys[0] });
  }

  hadleURISearch = (selectedKeys, confirm) => () => {
    confirm();
    ...
    const titleSearch = ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
        <div className={styles.customFilterDropdown}>
          <Input
            ...
            onPressEnter={this.hadleTitleSearch(selectedKeys, confirm)}
          />
          <Button
            type="primary"
            onClick={this.hadleTitleSearch(selectedKeys, confirm)}
          >
            Search
          </Button>
          <Button onClick={this.handleTitleReset(clearFilters)}>Reset</Button>
        </div>
      );
    ...
    const uriSearch = ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
        <div className={styles.customFilterDropdown}>
          <Input
            ...
            value={selectedKeys[0]}
            onChange={e => setSelectedKeys(e.target.value ? [e.target.value] : [])}
            onPressEnter={this.hadleURISearch(selectedKeys, confirm)}
          />
          <Button
            type="primary"
            onClick={this.hadleURISearch(selectedKeys, confirm)}
          >
            Search
          </Button>
          <Button onClick={this.handleURIReset(clearFilters)}>Reset</Button>
        </div>
      );
